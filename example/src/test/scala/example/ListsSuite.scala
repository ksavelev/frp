...
class ListsSuite extends FunSuite {
   ...
   */
  test("sum of a few numbers with 0") {
  ...
  test("sum of a two equal numbers with different sign") {
    assert(sum(List(-1,1)) === 0)
  }

  test("sum of a list with one element") {
    assert(sum(List(4)) === 4)
  }

  test("sum of a empty is 0") {
    assert(sum(List()) === 0)
  }

  test("max of a few numbers with 0") {
    assert(max(List(0, -2, -3)) === 0)
  }

  test("max of a few numbers with negative") {
    assert(max(List(3, 7, 2, -3)) === 7)
  }

  test("max throws NoSuchElementException if its argument is an empty list") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max of one positive numbers") {
    assert(max(List(3)) === 3)
  }

  test("max of one negative numbers") {
    assert(max(List(-3)) === -3)
  }

  test("max of some negative numbers") {
    assert(max(List(-3, -4, -2)) === -2)
  }
}
